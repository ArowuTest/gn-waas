...
func (r *GWLCaseRepository) GetMonthlyReport(ctx context.Context, period time.Time, districtID *uuid.UUID) (map[string]interface{}, error) {
	...
	err := r.q(ctx).QueryRow(ctx, fmt.Sprintf(`
		SELECT
			...
			COUNT(*) FILTER (WHERE COALESCE(af.gwl_status,'PENDING_REVIEW') IN ('CORRECTED','CLOSED'))            AS resolved,
			COUNT(*) FILTER (WHERE COALESCE(af.gwl_status,'PENDING_REVIEW') NOT IN ('CORRECTED','CLOSED','DISPUTED')) AS pending,
			COUNT(*) FILTER (WHERE COALESCE(af.gwl_status,'PENDING_REVIEW') = 'DISPUTED')                         AS disputed,
			COALESCE(SUM(af.estimated_loss_ghs) FILTER (WHERE af.anomaly_type::text != 'OVERBILLING'), 0) AS underbilling,
			COALESCE(SUM(af.estimated_loss_ghs) FILTER (WHERE af.anomaly_type::text = 'OVERBILLING'), 0)  AS overbilling,
			COALESCE(SUM(af.estimated_loss_ghs) FILTER (WHERE COALESCE(af.gwl_status,'PENDING_REVIEW') = 'CORRECTED'), 0)   AS recovered,
